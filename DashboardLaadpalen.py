...
for index, row in sjoin.iterrows():
    if row['PROV_NAAM'] == 'Limburg':
        folium.Marker(location=[row['Address_Latitude'], row['Address_Longitude']], tooltip = row['Address_Title'],
                      popup = '<b>Adres:</b><br>' + str(row['Address_AddressLine1']) + '<br>' + str(row['Address_Postcode']) + '<br>' + str(row['Address_Town']) + '<br><br>' + '<b>Prijs: </b>' + str(row['UsageCost']) + '<br><br>' + '<b>Type aansluiting: </b>' + '\n'.join(map(str, row['ConnectionType']) + '<br><br>' + '<b>Stroomtype: </b>' + str(row['CurrentType label']) + '<br><br>' + '<b>Aantal laadpunten: </b>' + str(row['NumberOfPoints']),
